...
class Session:
    ...
    def replace_message(self, message_id: str, message: Message) -> None:
        """Replace an existing message entry and rewrite the session file."""
        ...
        updated_message = message.model_copy(
            update={
                "id": entry.message.id,
                "parent_id": entry.message.parent_id,
                "timestamp": entry.message.timestamp,
            }
        )
        updated_entry = entry.model_copy(update={"message": updated_message})

        idx = self.entries.index(entry)
        self.entries[idx] = updated_entry
        self._entries_by_id[message_id] = updated_entry
        self._rewrite_file()
        self._rebuild_messages()
    ...
    def fork(self, from_message_id: str, session_dir: Path) -> Self:
        ...
        selected_provider, selected_model = self.get_model_selection() or (
    ...
    def active_entries(self) -> list[SessionEntry]:
        """Return entries on the active branch."""
        return list(self._branch_entries(self._leaf_id))
