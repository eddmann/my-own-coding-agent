...
@pytest.mark.asyncio
async def test_anthropic_retries_on_transient_error() -> None:
    call_count = {"count": 0}
    input_tokens = 1
    output_tokens = 1
...
@pytest.mark.asyncio
async def test_anthropic_provider_list_models_returns_registry_models() -> None:
...
def test_anthropic_provider_set_model_updates_and_rejects_cross_provider_models() -> None:
    provider = AnthropicProvider(api_key="sk-ant-test", model="claude-sonnet-4-20250514")
