...
from agent.llm.openai_codex.provider import DEFAULT_CODEX_INSTRUCTIONS
...
async def test_openai_codex_sends_expected_headers_and_path() -> None:
    ...
    assert captured["json"]["instructions"] == DEFAULT_CODEX_INSTRUCTIONS


@pytest.mark.asyncio
async def test_openai_codex_uses_oauth_file_and_refreshes_token(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("agent.llm.openai_codex.provider.refresh_openai_codex_token", fake_refresh)
